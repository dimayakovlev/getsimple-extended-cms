...
jQuery(document).ready(function () {
	...
	$("#addcomponent").live("click", function ($e) {
		...
		loadingAjaxIndicator.show();
		var id = $("#id").val();
		var component = $('<div style="display:none;" class="compdiv" id="section-' + id + '"><table class="comptable"><tr><td><b>' + GS.i18n['TITLE'] + ': </b><input type="text" class="text newtitle" name="components[' + id + '][title]" value="" /></td><td class="delete"><a href="#" title="' + GS.i18n['DELETE_COMPONENT'] + '?" class="delcomponent" id="del-' + id + '" rel="' + id + '" >&times;</a></td></tr><tr><td colspan="3" class="inline"><input type="checkbox" name="components[' + id + '][enabled]" value="1">&nbsp;<label for="components[' + id + '][enabled]">' + GS.i18n['ENABLE_COMPONENT'] + '</label></td></tr></table><textarea class="text" name="components[' + id + '][value]"></textarea><input type="hidden" name="components[' + id + '][slug]" value="" /><input type="hidden" name="components[' + id + '][id]" value="' + id + '" /><div>')
