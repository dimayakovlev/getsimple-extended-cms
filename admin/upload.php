...
if (isset($_FILES['file'])) {
	...
	// after uploading all files process messages
		if (sizeof($messages) != 0) $success = implode('<br>', $messages);
		if (sizeof($errors) != 0) $error = implode('<br>', $errors);
	}
}
...
if (isset($_GET['newfolder'])) {
	...
	$newfolder = filter_input(INPUT_GET, 'newfolder');
...
<div class="bodycontent">
	<div id="maincontent">
		...
		<?php
			...
			echo '</div><div id="new-folder"><a href="#" id="createfolder">' . i18n_r('CREATE_FOLDER') . '</a>';
			echo '<form action="upload.php">&nbsp;<input type="hidden" name="path" value="' . $subPath . '"><input type="hidden" name="nonce" value="' . get_nonce("createfolder") . '"><input type="text" class="text" name="newfolder" id="foldername"> <input type="submit" class="submit" value="' . i18n_r('CREATE_FOLDER') . '">&nbsp; <a href="#" class="cancel">' . i18n_r('CANCEL') . '</a></form>';
			echo '</div></div>';
			echo '<table class="highlight" id="imageTable">';
			echo '<tr><th class="imgthumb"></th><th>' . i18n_r('FILE_NAME') . '</th>';
			echo '<th class="size">' . i18n_r('FILE_SIZE') . '</th>';
			if (isDebug()) echo '<th class="perms">' . i18n_r('PERMS') . '</th>';
			echo '<th class="date">' . i18n_r('DATE') . '</th>';
			...
			if (count($dirsSorted) != 0) {
				...
				foreach ($dirsSorted as $upload) {
					...
					echo '<td class="size"><span>' . $directory_size . '</span></td>';
					// get the file permissions.
					if (isDebug()) {
						$filePerms = substr(sprintf('%o', fileperms($path . $upload['name'])), -4);
						if ($isUnixHost){
							$fileOwner = function_exists('posix_getpwuid') ? posix_getpwuid(fileowner($path . $upload['name'])) : array();
							$fileOwnerName = isset($fileOwner['name']) ? $fileOwner['name'] : '';
						} else {
							$fileOwnerName = getenv('USERNAME');
						}
						echo '<td class="perms"><span>' . $fileOwnerName . '/' . $filePerms . '</span></td>';
					}

					echo '<td class="date"><span>' . shtDate($upload['date']) . '</span></td>';
			...
			if (count($filesSorted) != 0) {
				foreach ($filesSorted as $upload) {
					$counter++;
					$cclass = ($upload['type'] == i18n_r('IMAGES') .' Images') ? 'iimage' : '';
					...
					echo '<td class="size"><span>' . $upload['size'] . '</span></td>';
	...
	<div id="sidebar"><?php include('template/sidebar-files.php'); ?></div>
