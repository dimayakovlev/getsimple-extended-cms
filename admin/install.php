...
foreach ($dirsArray as $dir) {
	...
	if (file_exists($dir)) {
		...
		if (!$result_755) {
			...
			if (!$result_777) $kill = i18n_r('CHMOD_ERROR');
		}
	} else {
		mkdir($dir, 0755);
		$result_755 = copy($tmpfile, $dir . 'tmp.tmp');
	...
	if (file_exists($dir . 'tmp.tmp')) unlink($dir . 'tmp.tmp');
}
...
foreach ($filenames as $lfile) {
	if (is_file(GSLANGPATH . $lfile) && $lfile != '.' && $lfile != '..') $lang_array[] = basename($lfile, '.php');
}
...
if (!file_exists($api_file)) {
	$saltval = defined('GSUSECUSTOMSALT') ? sha1(GSUSECUSTOMSALT) : generate_salt();
	$xml = new SimpleXMLExtended('<?xml version="1.0" encoding="UTF-8"?><item></item>');
	$note = $xml->addChild('apikey');
	$note->addCData($saltval);
	if (!XMLsave($xml, $api_file)) $kill = i18n_r('CHMOD_ERROR');
}
...
if (empty($APIKEY)) $kill = i18n_r('CHMOD_ERROR');
...
?>
	...
	<h3><?php echo $site_full_name . ' ' . i18n_r('INSTALLATION'); ?></h3>
		...
		<tr><td style="width:380px;"><?php echo $site_full_name; ?> <?php i18n_r('VERSION'); ?></td><td><span><strong><?php echo $site_version_no; ?></strong></span></td></tr>
		<tr><td>
		<?php
