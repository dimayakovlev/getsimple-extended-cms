...
*/
if (file_exists(GSPLUGINPATH)) $pluginfiles = getFiles(GSPLUGINPATH);

$pluginsLoaded = false;

// Check if data\other\plugins.xml exists
if (!file_exists(GSDATAOTHERPATH . 'plugins.xml')) create_pluginsxml();

read_pluginsxml(); // get the live plugins into $live_plugins array

// If not in frontend check that plugins have not been removed or added to the directory
if (!is_frontend()) create_pluginsxml();
 ...
 *
 * @param string $name
 * @param bool|null $active bool default=null, sets plugin active | inactive else toggle
 */
...
function create_pluginsxml($force = false) {
	...
	foreach ($pluginfiles as $fi) {
		if (lowercase(pathinfo($fi, PATHINFO_EXTENSION)) == 'php') $phpfiles[] = $fi;
	}
	if (!$force) {
		$livekeys = array_keys($live_plugins);
		if (count(array_diff($livekeys, $phpfiles)) > 0 || count(array_diff($phpfiles, $livekeys)) > 0) $force = true;
	}
	if ($force) {
...
function add_action($hook_name, $added_function, $args = array()) {
	...
	if (getDef('GSBTFIX', true) && version_compare(PHP_VERSION, '7.0.0', '>=')) $shift--;
...
function exec_action($a) {
	...
	foreach ($plugins as $hook) {
		if ($hook['hook'] == $a) call_user_func_array($hook['function'], $hook['args']);
	}
}
...
function createSideMenu($id, $txt, $action = null, $always = true) {
	$current = (isset($_GET['id']) && $_GET['id'] == $id && (!$action || isset($_GET[$action]))) ? true : false;
	if ($always || $current) echo '<li id="sb_' . $id . '" class="plugin_sb"><a href="load.php?id=' . $id . ($action ? '&amp;'.$action : '') . '" ' . ($current ? 'class="current"' : '') . '>' . $txt . '</a></li>';
}
 ...
 * @param string|callable $filter_function Function name or function to filter data
 */
function add_filter($filter_name, $filter_function) {
  global $filters;
  $bt = debug_backtrace();
  $caller = array_shift($bt);
  $pathName = pathinfo_filename($caller['file']);
...
function exec_filter($script, $data = array()) {
	...
	foreach ($filters as $filter) {
		if ($filter['filter'] == $script) $data = call_user_func_array($filter['function'], array($data));
	}
	return $data;
}
...
function deregister_script($handle) {
	global $GS_scripts;
	if (array_key_exists($handle, $GS_scripts)) unset($GS_scripts[$handle]);
}
...
function get_scripts_frontend($footer = false) {
	global $GS_scripts;
	if (!$footer) get_styles_frontend();
	foreach ($GS_scripts as $script) {
		if (($script['where'] & GSFRONT) && $script['load'] = true && script['in_footer'] == $footer) {
 ...
 * @since 3.1
 * @uses $GS_scripts
 * @uses $GS_asset_objects
 * @uses $GS_script_assets
 *
 * @param boolean $footer Load only script with footer flag set
 */
function get_scripts_backend($footer = false) {
	global $GS_scripts, $GS_asset_objects, $GS_script_assets;
	if (!$footer) get_styles_backend();
	# debugLog($GS_scripts);
	foreach ($GS_scripts as $script) {
		if (($script['where'] & GSBACK) && $script['load'] == true && $script['in_footer'] == $footer) {
			echo "\t<script src=\"" . $script['src'] . '?v='.$script['ver'] . "\"></script>\n";
			if (!getDef('GSNOCDN', true) && ($script['name'] == 'jquery' || $script['name'] == 'jquery-ui')) {
				echo "\t<script>";
...
function get_styles_frontend() {
	...
	foreach ($GS_styles as $style) {
		if (($style['where'] & GSFRONT) && $style['load'] == true) echo "\t" . '<link href="' . $style['src'] . '?v='.$style['ver'] . '" rel="stylesheet" media="' . $style['media'] . "\">\n";
	}
}
...
function get_styles_backend() {
	global $GS_styles;
	foreach ($GS_styles as $style) {
		if (($style['where'] & GSBACK) && $style['load'] == true) echo "\t" . '<link href="' . $style['src'] . '?v=' . $style['ver'] . '" rel="stylesheet" media="' . $style['media'] . "\">\n";
	}
}
