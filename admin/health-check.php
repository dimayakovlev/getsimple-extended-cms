	...
	<div id="maincontent">
		<div class="main">
			...
			<table class="highlight healthcheck">
				...
				<?php
					...
					if (version_compare(PHP_VERSION, "5.3", "<")) {
						echo 'PHP ' . i18n_r('VERSION') . '</td><td><span class="ERRmsg"><b>' . PHP_VERSION . '</b> - PHP 5.3 ' . i18n_r('OR_GREATER_REQ') . ' - ' . i18n_r('ERROR') . '</span></td></tr>';
					} else {
					...
					if (server_is_apache()) {
						...
						if (!defined('GSNOAPACHECHECK') || GSNOAPACHECHECK == false) echo '<tr><td>Apache web server</td><td><span class="ERRmsg">' . $_SERVER['SERVER_SOFTWARE'] . ' - <b>' . i18n_r('ERROR') . '</b></span></td></tr>';
					}
			...
			<?php
				...
				sort($data);
				foreach ($data as $file) {
					if(isFile($file, $path)) echo '<tr><td>/data/other/' . $file . '</td><td>' . valid_xml($path . $file) . '</td></tr>';
				}
				...
				sort($data);
				foreach ($data as $file) {
					if (isFile($file, $path, '.log')) echo '<tr><td>/data/other/logs/' . $file . '</td><td>' . valid_xml($path . $file) . '</td></tr>';
				}
				...
				sort($data);
				foreach ($data as $file) {
					if (isFile($file, $path)) echo '<tr><td>/backups/users/' . $file . '</td><td>' . valid_xml($path . $file) . '</td></tr>';
				}
			?>
			</table>
			...
			<table class="highlight healthcheck">
				...
				<?php
					...
					if (!file_exists($file)) copy(GSADMININCPATH . 'tmp/tmp.allow.htaccess', $file);
					if (!file_exists($file)) {
						echo ' <span class="WARNmsg">' . i18n_r('MISSING_FILE') . ' - ' . i18n_r('WARNING') . '</span>';
					} else {
						$res = file_get_contents($file);
